...
export async function GET(_request: Request, { params }: Props) {
  ...
  if (!markdown) {
    return new Response(`# Página no encontrada\n\nConsulta ${siteConfig.url}/sitemap.xml o ${siteConfig.url}/llms.txt.\n`, {
      status: 404,
      headers: { "Content-Type": "text/markdown; charset=utf-8", Vary: "Accept", "X-Robots-Tag": "noindex" },
    });
  }
  ...
  return new Response(markdown, {
    headers: {
      ...
      Link: `<${canonical}>; rel="canonical", <${markdownUrl(path)}>; rel="alternate"; type="text/markdown", <${siteConfig.url}/llms.txt>; rel="describedby"`,
